...
void main() {
  ...
  group('Phase 10 — Production Hardening & Security Audit Tests', () {
    test('1. Security Scan: Zero Hardcoded Secret Credentials in Source Code', () {
      ...
      final secretRegex = RegExp(r'(r2_access_key|r2_secret_key|aws_secret_access_key|private_key|api_secret)\s*=\s*["\x27][^"\x27]+["\x27]', caseSensitive: false);

      for (final file in files) {
        final content = file.readAsStringSync();
        expect(secretRegex.hasMatch(content), isFalse,
            reason: 'File ${file.path} contains forbidden hardcoded secret credentials!');
      }
    });

    test('2. Network Security Scan: Zero Insecure http:// URLs in Catalog & Data Models', () {
    ...
    test('4. PdfRepository Validation: Zero Scheme Errors & Zero Duplicate Resource IDs', () {
      final catalog = AcademicResourceCatalog.allAcademicResources;
      final pdfRepo = PdfRepository();
      final errors = pdfRepo.validateDocumentUrls(catalog);
      expect(errors, isEmpty, reason: 'PdfRepository URL validation must return 0 scheme errors and 0 duplicate IDs');
    });
    ...
    test('6. Academic Hierarchy Integrity Verification (4 Years, 8 Semesters, 30 Subjects)', () {
      ...
      final sem1Subjects = MockData.subjects.where((s) => s.semesterId == 'sem_1_1').toList();
      expect(sem1Subjects, isNotEmpty);

      final sem2Subjects = MockData.subjects.where((s) => s.semesterId == 'sem_2_1').toList();
      expect(sem2Subjects, isNotEmpty);

      final sem3Subjects = MockData.subjects.where((s) => s.semesterId == 'sem_3_1').toList();
      expect(sem3Subjects, isNotEmpty);
    });

    test('7. Non-Academic Educational Hubs Structural Integrity Verification (72 Topics)', () {
