...
void main() {
  ...
  group('Phase 11 — Production Cloud Storage & Integration Tests', () {
    test('1. AppConfig environment switching & cdnBaseUrl getters', () {
      ...
      expect(AppConfig.cdnBaseUrl,
          equals('https://staging-cdn.csse-study-hub.org/academic'));

      AppConfig.environment = AppEnvironment.dev;
      expect(AppConfig.cdnBaseUrl,
          equals('https://dev-cdn.csse-study-hub.org/academic'));
    ...
    test('2. R2StorageHelper path generator and CDN URL builder', () {
      ...
      final cdnUrl = R2StorageHelper.buildCdnUrl(path,
          customCdnBaseUrl: 'https://cdn.csse-study-hub.org/academic');
      expect(
          cdnUrl,
          equals(
              'https://cdn.csse-study-hub.org/academic/year_1/sem_1_1/cs1104/unit_1/notes/c_syntax.pdf'));
    });

    test('3. CloudStorageHealthChecker handles invalid/empty URLs gracefully',
        () async {
      final checker = CloudStorageHealthChecker();
      final invalidResult = await checker
    ...
    test(
        '4. AcademicResourceCatalog production HTTPS validation & zero duplicates',
        () {
      ...
      expect(errors, isEmpty,
          reason:
              'Catalog must contain 100% valid HTTPS schemes and 0 duplicate IDs');
    });
    ...
    test(
        '6. Security Audit: Zero hardcoded secrets/credentials in source files',
        () {
      ...
      for (final file in files) {
        ...
        expect(secretPattern.hasMatch(text), isFalse,
            reason:
    ...
    test(
        '7. Protected Academic Hierarchy Verification (4 Years, 8 Semesters, 30 Subjects)',
        () {
