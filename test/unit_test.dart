...
void main() {
  group('CSSE Study Hub Unit Tests', () {
    ...
    test(
        'CareerRepository returns offline fallback data when remote is unavailable',
        () async {
      ...
      expect(techList.any((t) => t.name.contains('Artificial Intelligence')),
          true);
    });

    test('CodingRepository returns offline fallback coding resources',
        () async {
    ...
    test('PlacementRepository returns aptitude & interview preparation items',
        () async {
      final repo =
          PlacementRepository(firebaseDataSource: mockFirebaseDataSource);
    ...
    test('ProjectRepository returns mini and major projects', () async {
      final repo =
          ProjectRepository(firebaseDataSource: mockFirebaseDataSource);
    ...
    test('HigherEducationRepository returns higher studies & govt exam guides',
        () async {
      final repo =
          HigherEducationRepository(firebaseDataSource: mockFirebaseDataSource);
