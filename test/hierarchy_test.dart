...
void main() {
  group('Non-Academic Hierarchy Data & Repository Tests', () {
    ...
    test(
        'Flow 1 Data Path Verification (Coding -> Python -> Beginner -> Lists -> Learn Online)',
        () {
      final codingHub = repository.getHubById('coding');
      expect(codingHub, isNotNull);

      final langCat =
          repository.getCategoryById('coding', 'programming_languages');
      expect(langCat, isNotNull);

      final pythonTopic =
          repository.getTopicById('coding', 'programming_languages', 'python');
      expect(pythonTopic, isNotNull);

      // Subtopics under Python -> Data Structures
      final dsTopic =
          pythonTopic!.subtopics.firstWhere((t) => t.id == 'python_ds');
      expect(dsTopic, isNotNull);

      // Subtopics under Data Structures -> Lists
      final listsTopic =
          dsTopic.subtopics.firstWhere((t) => t.id == 'python_ds_lists');
    ...
    test(
        'Flow 2 Data Path Verification (Emerging Tech -> AI -> ML -> Beginner -> Learn Online)',
        () {
      ...
      final mlTopic =
          repository.getTopicById('emerging_tech', 'ai_ml', 'machine_learning');
      expect(mlTopic, isNotNull);

      final mlIntro =
          mlTopic!.subtopics.firstWhere((t) => t.id == 'ml_intro_beginner');
    ...
    test(
        'Flow 3 Data Path Verification (Placement -> Tech Prep -> DSA -> Practice)',
        () {
      ...
      final dsaTopic =
          repository.getTopicById('placement', 'tech_prep', 'placement_dsa');
      expect(dsaTopic, isNotNull);

      final dsaTopTopic = dsaTopic!.subtopics
          .firstWhere((t) => t.id == 'dsa_top_interview_topic');
    ...
    test(
        'Phase 7: SearchIndexEngine indexes all 5 non-academic hubs & search queries work',
        () {
      ...
      expect(
          pythonResult.matchingItems.any((i) =>
              i.title.contains('Python') || i.keywords.contains('Python')),
          isTrue);

      // Search 2: Emerging Tech (Machine Learning)
      final mlResult = engine.search('Machine Learning');
      expect(
          mlResult.matchingItems.any((i) =>
              i.title.contains('Machine Learning') ||
              i.subtitle.contains('Emerging')),
          isTrue);

      // Search 3: Higher Education (GATE)
      final gateResult = engine.search('GATE');
      expect(
          gateResult.matchingItems.any(
              (i) => i.title.contains('GATE') || i.keywords.contains('GATE')),
          isTrue);

      // Search 4: Placement (Array Basics / Aptitude)
      final aptitudeResult = engine.search('Aptitude');
      expect(
          aptitudeResult.matchingItems.any((i) =>
              i.title.contains('Aptitude') || i.keywords.contains('Aptitude')),
          isTrue);

      // Search 5: Projects & Practice (To-Do List)
      final todoResult = engine.search('To-Do List');
      expect(
          todoResult.matchingItems.any((i) =>
              i.title.contains('To-Do') || i.keywords.contains('To-Do List')),
          isTrue);
    });

    test(
        'Phase 7: BookmarkProvider & RecentProvider support non-academic items without breaking Academic items',
        () async {
      ...
      expect(
          recentProvider.recentResources
              .any((r) => r.resourceId == 'res_python_basics_test'),
          isTrue);
      ...
      expect(
          recentProvider.recentResources
              .any((r) => r.resourceId == 'res_c_pointers_acad'),
          isTrue);
    });

    test(
        'Phase 9: Complete Content Quality & Data Consistency Audit Across All Hubs',
        () {
      final hubs = repository.getAllHubs();
      expect(hubs.length, greaterThanOrEqualTo(5));
      ...
      void checkTopic(HierarchicalTopicModel topic) {
        ...
        expect(globalIds.contains(topic.id), isFalse,
            reason: 'Duplicate topic ID found: ${topic.id}');
        ...
        for (final res in topic.resources) {
          ...
          expect(globalIds.contains(res.id), isFalse,
              reason: 'Duplicate resource ID found: ${res.id}');
          ...
          expect(
              ...
              isTrue,
              reason: 'Invalid URL format: ${res.url}');
        }
      ...
      for (final hub in hubs) {
        ...
        expect(globalIds.contains(hub.id), isFalse,
            reason: 'Duplicate hub ID found: ${hub.id}');
          ...
          for (final step in hub.startHereSteps!) {
            ...
            if (step.targetCategoryId != null) {
              expect(
                ...
                isTrue,
                reason:
                    'StartHereStep targetCategoryId "${step.targetCategoryId}" does not exist in hub "${hub.id}"',
              );
            }
          }
        }
        ...
        for (final cat in hub.categories) {
          ...
          expect(globalIds.contains(cat.id), isFalse,
              reason: 'Duplicate category ID found: ${cat.id}');
