...
void main() {
  ...
  group('Phase 9 — Production Academic Resource Delivery Tests', () {
    ...
    test('Verify AcademicResourceCatalog returns recommended Start Here resource for subjects', () {
      final cProgRecommended = AcademicResourceCatalog.getRecommendedResource('subj_1_1_4');
      ...
      final dsRecommended = AcademicResourceCatalog.getRecommendedResource('subj_2_1_1');
    ...
    test('Verify Copyright Tier Safety Rules (external_copyrighted vs hosted)', () {
      final kernelDocs = catalog.firstWhere((r) => r.id == 'pdf_os_official_docs_kernel');
      ...
      final cNotes = catalog.firstWhere((r) => r.id == 'pdf_c_prog_start_here_unit1');
    ...
    test('Verify Availability Status Handling (available vs coming_soon)', () {
      final availableRes = catalog.firstWhere((r) => r.id == 'pdf_c_prog_start_here_unit1');
      expect(availableRes.availabilityStatus, equals('available'));
      expect(availableRes.isComingSoon, isFalse);

      final comingSoonRes = catalog.firstWhere((r) => r.id == 'pdf_c_prog_unit4_coming_soon');
      expect(comingSoonRes.availabilityStatus, equals('coming_soon'));
      expect(comingSoonRes.isComingSoon, isTrue);
    });

    test('Verify PdfRepository URL validation returns zero errors on AcademicResourceCatalog', () {
      final pdfRepo = PdfRepository();
      final errors = pdfRepo.validateDocumentUrls(catalog);
      expect(errors, isEmpty, reason: 'Catalog URLs must be valid HTTPS and unique');
    });
    ...
    test('Verify Academic Hierarchy Preservation (4 Years, 8 Semesters, 30 Subjects)', () {
