...
class _HigherEdDetailScreenState extends State<HigherEdDetailScreen> {
        ...
        body: Column(
          children: [
            ...
            Expanded(
              child: TabBarView(
                children: [
                  _buildOverviewTab(context, item, cardBg, borderColor,
                      textPrimary, textSecondary, isDark),
                  _buildSyllabusBooksTab(context, item, cardBg, borderColor,
                      textPrimary, textSecondary, isDark),
                  _buildRoadmapTab(context, item, cardBg, borderColor,
                      textPrimary, textSecondary, isDark),
                  _buildApplicationTab(context, item, cardBg, borderColor,
                      textPrimary, textSecondary, isDark),
                  _buildOfficialSourcesTab(context, item, cardBg, borderColor,
                      textPrimary, textSecondary, isDark),
                ],
              ),
            ),
  ...
  Widget _buildOverviewTab(
    ...
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        _buildCard(cardBg, borderColor,
            child: Text(item.eligibilityCriteria,
                style: TextStyle(
                    color: textSecondary, fontSize: 13, height: 1.4))),
        if (item.whoShouldApply.isNotEmpty) ...[
          const SizedBox(height: 18),
          _buildSectionHeader(
              'Target Audience / Who Should Apply', Icons.groups_rounded),
          const SizedBox(height: 8),
          _buildCard(cardBg, borderColor,
              child: Text(item.whoShouldApply,
                  style: TextStyle(
        ...
        if (item.careerOpportunities.isNotEmpty) ...[
          const SizedBox(height: 18),
          _buildSectionHeader(
              'Career Opportunities & Scope', Icons.trending_up_rounded),
          const SizedBox(height: 8),
          _buildCard(cardBg, borderColor,
              child: Text(item.careerOpportunities,
                  style: TextStyle(
                      color: textSecondary, fontSize: 13, height: 1.4))),
        ],
      ],
    ...
    return ListView(
      ...
      children: [
        if (item.examPattern.isNotEmpty) ...[
          _buildSectionHeader(
              'Exam Pattern & Marking Scheme', Icons.assignment_rounded),
          const SizedBox(height: 8),
          _buildCard(cardBg, borderColor,
              child: Text(item.examPattern,
                  style: TextStyle(
                      color: textPrimary, fontSize: 13, height: 1.4))),
          const SizedBox(height: 18),
        ],
        ...
        if (item.recommendedBooks.isNotEmpty) ...[
          const SizedBox(height: 18),
          _buildSectionHeader(
              'Recommended Reference Textbooks', Icons.library_books_rounded),
          const SizedBox(height: 8),
  ...
  Widget _buildRoadmapTab(
    ...
    return ListView(
      ...
      children: [
        _buildSectionHeader(
            'Phased Preparation Timeline', Icons.timeline_rounded),
        const SizedBox(height: 10),
        ...
        if (item.successStrategy.isNotEmpty) ...[
          ...
          const SizedBox(height: 8),
          _buildCard(cardBg, borderColor,
              child: Text(item.successStrategy,
                  style: TextStyle(
                      color: textPrimary, fontSize: 13, height: 1.4))),
        ],
      ],
  ...
  Widget _buildApplicationTab(
    ...
    return ListView(
      ...
      children: [
        _buildSectionHeader(
            'Application Process & Guidelines', Icons.app_registration_rounded),
        const SizedBox(height: 8),
        _buildCard(cardBg, borderColor,
            child: Text(item.applicationProcess,
                style:
                    TextStyle(color: textPrimary, fontSize: 13, height: 1.4))),
        if (item.topInstitutes.isNotEmpty) ...[
          const SizedBox(height: 18),
          _buildSectionHeader(
              'Top Premier Institutes & Colleges', Icons.domain_rounded),
          const SizedBox(height: 8),
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: item.topInstitutes.map((inst) {
              return Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border:
                      Border.all(color: const Color(0xFF4F46E5).withAlpha(40)),
                ),
                child: Text(inst,
                    style: const TextStyle(
        ...
        if (item.scholarships.isNotEmpty) ...[
          const SizedBox(height: 18),
          _buildSectionHeader(
              'Available Scholarships & Stipends', Icons.payments_rounded),
          const SizedBox(height: 8),
  ...
  Widget _buildOfficialSourcesTab(
      BuildContext context,
      HigherEducationModel item,
      Color cardBg,
      Color borderColor,
    ...
    return ListView(
      ...
      children: [
        _buildSectionHeader(
            'Verified Official Websites', Icons.verified_user_rounded),
        const SizedBox(height: 10),
        ...
        if (item.latestNotifications.isNotEmpty) ...[
          const SizedBox(height: 18),
          _buildSectionHeader('Official Notifications & Schedule',
        ...
        if (item.faqs.isNotEmpty) ...[
          const SizedBox(height: 18),
          _buildSectionHeader(
              'Frequently Asked Questions (FAQs)', Icons.quiz_rounded),
          const SizedBox(height: 10),
